...
def upload_detail(request, pk):
	...
	for efont in font_list:
		stored_font.append({
			"title": efont.title,
			"similarity": jacsim(upload_font, econversion(efont.font.path))	
		})
	
	
	return render(request, 'blog/upload_detail.html', {
			'upload' : upload, 'sf' : stored_font,  

	})
# Whole Function upload_detail is the responsible of Semin Han
